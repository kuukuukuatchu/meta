...
if unit.exists('target') then
    ...
    -- Fiery Brand
        ...
        if cast.check(fieryBrand) and not buff.exists('player',buff.demonSpikes) and not buff.exists('player',buff.metamorphosis) then
            cast.fieryBrand()
        end
    -- Demon Spikes
        -- actions+=/demon_spikes,if=charges=2|buff.demon_spikes.down&!dot.fiery_brand.ticking&buff.metamorphosis.down
        if cast.check(demonSpikes) and (spell.charges(demonSpikes) == 2 or not buff.exists('player',buff.demonSpikes)) and not debuff.exists('target',debuff.fieryBrand) and not buff.exists('player',buff.metamorphosis) then
            cast.demonSpikes()
        end
    -- Infernal Strike
    ...
    -- Spirit Bomb
        ...
        if cast.check(spiritBomb) and not debuff.exists('target',debuff.frailty) then
            cast.spiritBomb()
        end
    -- Immolation Aura
    ...
    -- Soul Cleave
        ...
        if cast.check(soulCleave) and buff.stack('player',buff.soulFragments) == 5 then
            cast.soulCleave()
        end
    -- Metamorphosis
        ...
        if cast.check(metamorphosis) and not buff.exists('target',buff.demonSpikes)
